...
def sunday_event(local_appObj, local_fileObj, time_start, time_end):
    schedule.every().sunday.at(time_start).do(startRecord, local_appObj, local_fileObj)
    schedule.every().sunday.at(time_end).do(endRecord, local_appObj, local_fileObj)

    schedule.every().sunday.at(time_start).do(startStream, local_appObj, local_fileObj)
    schedule.every().sunday.at(time_end).do(endStream, local_appObj, local_fileObj)
...
def tuesday_event(local_appObj, local_fileObj, time_start, time_end):
    schedule.every().tuesday.at(time_start).do(startRecord, local_appObj, local_fileObj)
    schedule.every().tuesday.at(time_end).do(endRecord, local_appObj, local_fileObj)

    schedule.every().tuesday.at(time_start).do(startStream, local_appObj, local_fileObj)
    schedule.every().tuesday.at(time_end).do(endStream, local_appObj, local_fileObj)

def wednesday_event(local_appObj, local_fileObj, time_start, time_end):
    schedule.every().wednesday.at(time_start).do(startRecord, local_appObj, local_fileObj)
    schedule.every().wednesday.at(time_end).do(endRecord, local_appObj, local_fileObj)

    schedule.every().wednesday.at(time_start).do(startStream, local_appObj, local_fileObj)
    schedule.every().wednesday.at(time_end).do(endStream, local_appObj, local_fileObj)

def thursday_event(local_appObj, local_fileObj, time_start, time_end):
    schedule.every().thursday.at(time_start).do(startRecord, local_appObj, local_fileObj)
    schedule.every().thursday.at(time_end).do(endRecord, local_appObj, local_fileObj)

    schedule.every().thursday.at(time_start).do(startStream, local_appObj, local_fileObj)
    schedule.every().thursday.at(time_end).do(endStream, local_appObj, local_fileObj)

def friday_event(local_appObj, local_fileObj, time_start, time_end):
    schedule.every().friday.at(time_start).do(startRecord, local_appObj, local_fileObj)
    schedule.every().friday.at(time_end).do(endRecord, local_appObj, local_fileObj)

    schedule.every().friday.at(time_start).do(startStream, local_appObj, local_fileObj)
    schedule.every().friday.at(time_end).do(endStream, local_appObj, local_fileObj)

def saturday_event(local_appObj, local_fileObj, time_start, time_end):
    schedule.every().saturday.at(time_start).do(startRecord, local_appObj, local_fileObj)
    schedule.every().saturday.at(time_end).do(endRecord, local_appObj, local_fileObj)

    schedule.every().saturday.at(time_start).do(startStream, local_appObj, local_fileObj)
    schedule.every().saturday.at(time_end).do(endStream, local_appObj, local_fileObj)
...
def main(value):
    ...
    with open("Event.txt") as infile:
        reader = csv.reader(infile)
        next(reader)
        

    #Parse contents of the csv and call the scheduler functions
